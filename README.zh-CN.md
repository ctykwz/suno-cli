...
- 普通音频检查优先使用现有 clip 媒体：`sunox clip info <id> --json` 会给出 `audio_url`，`sunox clip download` 当前从 `clip.audio_url` 下载 MP3，`--video` 只在有 `clip.video_url` 时使用。`sunox clip stems` 是生成式分轨，不等同于 Suno Web 的 Pro Get Stems 导出。Suno Web 还显示 WAV Audio、Get Stems、Video 等 Pro 下载选项；只有 CLI 明确暴露支持且用户明确要求对应格式时，agent 才应使用这些路径。若 `playlist remove` 返回 `partial_mutation`，重试前先查看 `error.details.succeeded_clip_ids`、`error.details.failed` 和 `error.details.not_attempted_clip_ids`。
...
|---|---|---|
| 0 | 成功 | 继续 |
| 1 | 运行时、Web 接口或部分写入失败 | 先查看 `error.code` 和 `error.details` 再决定是否重试 |
| 2 | 配置错误 | 修复配置，不要盲目重试 |
